...
public class TableModel extends AbstractTableModel<Reserva> implements javax.swing.table.TableModel {
    ...
    protected Object getPropetyAt(Reserva e, int col) {
        switch (cols[col]) {
            ...
            case FECHA: return e.getFecha() != null ? e.getFecha().toString() : "";
            case HORARIO: return (e.getHoraInicio() != null ? e.getHoraInicio().format(HH_MM) : "") + " - "
                    + (e.getHoraFin() != null ? e.getHoraFin().format(HH_MM) : "");
            case RECURSOS: return e.getRecursosAsignados().stream().map(Recurso::getId).collect(Collectors.joining(", "));
            case ESTADO: return e.getEstado() != null ? e.getEstado().toString() : "";
            default: return "";
        }
    }

    public void setRows(List<Reserva> rows) {
        this.rows = rows;
